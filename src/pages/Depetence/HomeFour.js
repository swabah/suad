  ...
  return (
    <div   className='bg-fixed bg-cover bg-brightness-50 bg-no-repeat w-full h-full'>
     ...
     <div className='w-full flex items-center md:pt-1  pb-2 justify-between'>
       <h2 className='text-2xl lg:text-3xl xl:text-4xl font-bold'>How Ahlussuffa Distinct from others?</h2>
     </div>
     <div className='pt-1 md:pt-8 w-full h-auto grid grid-cols-1 grid-rows-auto gap-y-1 md:gap-y-4 items-center justify-center'>
        {data.map((item)=>(
          <h2 className='text-base lg:text-lg w-full h-auto md:px-3 lg:px-5 py-3 lg:py-5 md:drop-shadow-lg border-b md:border-t md:shadow-md rounded md:uppercase' >{item.item}</h2>
          ))}
     </div>
   </div>
    </div>
